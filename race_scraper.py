...
# スレッドローカルに会場コードを保持（複数タブの同時アクセスで干渉しない）
_thread_local = threading.local()


def set_thread_venue(jycd: str):
    """現在のスレッドの会場コードを設定する（Streamlitセッション単位で安全）。"""
    _thread_local.jycd = jycd


def _jycd():
    """現在のスレッドの会場コードを返す。"""
    return getattr(_thread_local, "jycd", _cfg.JYCD)


def _venue_executor(max_workers: int) -> ThreadPoolExecutor:
    """現在のスレッドの会場コードをワーカーに伝播する ThreadPoolExecutor を返す。"""
    vc = _jycd()
    return ThreadPoolExecutor(max_workers=max_workers, initializer=set_thread_venue, initargs=(vc,))
...
def fetch_before_info(race_no: int, date_str: str | None = None) -> tuple[pd.DataFrame, dict]:
    ...
    if _cfg.get_venue_config(_jycd()).get("has_iot_weather"):
    ...
    if _cfg.get_venue_config(_jycd()).get("has_official_weather"):
        _venue_code = _cfg.get_venue_config(_jycd()).get("code", "")
        if _venue_code == "12":
...
def fetch_extended_player_data(reg_nos: list[str]) -> dict:
    ...
    try:
        with _venue_executor(6) as executor:
