...
【使い方】
1. データ収集:        python tune_amagasaki.py collect --days 30
2. チューニング:      python tune_amagasaki.py tune
3. 収集＋チューニング: python tune_amagasaki.py all --days 30
...
AMAGASAKI_BACKTEST_FILE = PROJECT_DIR / "backtest_data_amagasaki.json"
AMAGASAKI_TUNE_RESULT_FILE = PROJECT_DIR / "_amagasaki_tune_result.json"
VENUE_CODE = "13"
...
def load_amagasaki_data() -> list[dict]:
    if AMAGASAKI_BACKTEST_FILE.exists():
        try:
            return json.loads(AMAGASAKI_BACKTEST_FILE.read_text(encoding="utf-8"))
        except Exception:
            return []
    return []


def _save_amagasaki_data(data: list[dict]) -> None:
    data.sort(key=lambda x: (x["date"], x["race_no"]))
    AMAGASAKI_BACKTEST_FILE.write_text(
...
def collect_amagasaki_data(days: int = 30, delay: float = 1.0) -> list[dict]:
    """
    尼崎の過去 days 日分のレースデータを BATCH_SIZE_DAYS 日ずつ収集する。
    """
...
def tune_amagasaki():
    """尼崎の過去データでパラメータチューニングを実行する。"""
    from config import VENUE_CONFIGS

    all_data = load_amagasaki_data()
    if len(all_data) < 20:
    ...
    AMAGASAKI_TUNE_RESULT_FILE.write_text(
        json.dumps(out, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(f"\n結果を {AMAGASAKI_TUNE_RESULT_FILE} に保存しました")

    # config.py に反映するためのコードスニペットを出力
    print("\n" + "=" * 60)
    print("config.py に反映するには以下を _AMAGASAKI_SCORE_WEIGHTS.update に追加:")
    print("=" * 60)
    print("_AMAGASAKI_SCORE_WEIGHTS.update({")
    for k, v in opt.items():
        print(f'    "{k}": {v},')
    print("})")
...
def main():
    parser = argparse.ArgumentParser(
        description="尼崎競艇場 データ収集＆チューニング")
    parser.add_argument(
    ...
    if args.command in ("collect", "all"):
        collect_amagasaki_data(days=args.days, delay=args.delay)
